...
trait FileTrait {
     ...
     * @return string
     */
    protected static function getDestinationFilePaths($file)
    {
...
// 
     ...
     * @return boolean
     */
    public function upload($file, $save = true, $quality = -1, array $overrideColumns = [])
    {
        // Get absolute and relative path
        $path = $this->getDestinationFilePaths($file);
        ...
        try {
            ...
            if($quality != -1 && in_array(strtolower($file->getClientOriginalExtension()), ['jpg', 'jpeg', 'png', 'gif'])) {
                $this->compress($absolutePathInfo['dirname'] . '/' . $absolutePathInfo['basename'], $quality);
            }
