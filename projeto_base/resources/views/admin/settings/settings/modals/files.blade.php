{{ Form::open(array('route' => array('admin.settings.storage.clean'))) }}
            ...
            <div class="col-sm-1" data-file="{{ $filepath }}" data-filename="{{ str_replace('.' . $file->getExtension(), '', $file->getFilename()) }}" data-size="{{ $file->getSize() }}" data-created="{{ $file->getMTime() }}">
                <div class="item">
                    <div class="item-preview">
                        ...
                        <a href="{{ @$storage ? route('admin.settings.file.download', ['file' => $filepath]) : asset($filepath) }}" target="_blank">
...
<div class="modal-footer">
    <a href="{{ route('admin.settings.directory.clean', ['folder' => $directoryName]) }}" data-method="delete"
    ...
    <a href="{{ route('admin.settings.directory.compact', ['folder' => $directoryName]) }}"
       data-method="post"
    ...
    {{--<a href="{{ route('admin.settings.file.obsolete', ['folder' => $directoryName]) }}" class="btn btn-sm btn-default pull-left btn-obsolete" data-loading-text="<i class='fas fa-spin fa-circle-notch'></i> A verificar...">
        <i class="fas fa-file-code-o"></i> Idêntificar Ficheiros Obsoletos
    </a>--}}
    <button type="button" class="btn btn-default" data-dismiss="modal">Fechar</button>
</div>
{{ Form::close() }}
@include('admin.settings.settings.modals.confirm')
<script>
    ...
    $('[data-confirm-btn]').on('click', function(){
        if($(this).data('confirm-btn') == '0') {
            ...
            $.post("{{ route('admin.settings.file.destroy') }}", {file:fileUrl}, function(){
