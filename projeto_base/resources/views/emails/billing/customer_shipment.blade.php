...
@section('content')
    ...
    @if(count($data['shipments']) > 1)
        <h5 style="font-size: 16px">Faturação de serviços</h5>
    @elseif(count($data['shipments']) == 1)
        <h5 style="font-size: 16px">Fatura do envio {{ @$data['shipment']->tracking_code }}</h5>
    @elseif(count($data['covenants']) >= 1)
        <h5 style="font-size: 16px">Fatura de Avença Mensal</h5>
    @endif
    <p>
        Estimado cliente,
        <br/>
